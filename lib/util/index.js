...
util.encodeNumber = function(n, trim){
  if(trim)
    return d64.encode(bytewise.encode(n)).replace(padX,'');
  else
    return d64.encode(bytewise.encode(n));
};
util.decodeNumber = function(str){
  return bytewise.decode(
    new Buffer(d64.decode(pad(str, 12)), 'binary')
  );
};
util.encodeClock = function(clock){
  return util.encode(
    util.decodeNumber(clock.slice(8))
  ) + clock.slice(0,8);
};
