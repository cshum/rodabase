...
module.exports = function(path, opts){
      ...
      function emit(name, key, value, unique){
        ...
        if(unique === true)
          tx.get(enKey, {
            ...
            if(val) tx.error(new Error(key + ' must be unique.'));
          });
  ...
  function(){
    ...
    return H(function(push){
      changes.post(function(data){
        if(data.value) //omit del operations
          push(null, data.value);
      });
    });
  };
