...
function indexer(snapshot, tx){
  ...
  var name, i, l, keys;
  if(snapshot.indexed){
    for(name in snapshot.indexed){
      keys = snapshot.indexed[name];
  ...
  if(snapshot.body._deleted) return; 

  var result = extend({}, snapshot.body);
  var indexed = {};
  var async = false;
  ...
  var emit = function(name, key, value, unique){
    ...
    tx.put(
      prefix(self._index, name, idx.map(codec.encode).join(LOW)), 
      extend({}, value || result, {
        _id: result._id, 
        _key: key 
      })
...
function operation(ctx, next){
  ...
  tx.get(prefix(self._snapshot, ctx.result._id), function(err, snapshot){
    //return IO/other errors
    if(err && !err.notFound) 
      return next(err);
    //has current
    snapshot = snapshot || {};
    ctx.current = snapshot.body;
    ...
    tx.get(prefix(self._clock, mid), function(err, curr){
      ...
      //clock update
      tx.put(prefix(self._clock, mid), seq);

      //put snapshot
      snapshot.body = ctx.result;
      ...
      //clean up timeline
      if(snapshot.time)
        tx.del(prefix(self._timeline, codec.encode(snapshot.time)));

      snapshot.time = timestamp();
      var time = extend({}, ctx.result);
      time._time = snapshot.time;

      //put timeline
      tx.put(prefix(self._timeline, codec.encode(snapshot.time)), time);
      ...
      //put snapshot
      tx.put(prefix(self._snapshot, ctx.result._id), snapshot);
      
      indexer.call(self, snapshot, tx);
...
module.exports = function(path, opts){
  ...
  Roda.fn.define('get', params('id','deleted:boolean?','tx?'), function(ctx, done){
    ...
    function callback(err, snapshot){
      if(err && err.notFound)
        return done(error.notFound(id));
      if(err) 
        return done(err);
      var doc = snapshot.body;
    ...
    if(ctx.params.tx){
      ...
      tx.get(prefix(this._snapshot, id), callback);
    }else{
      db.get(prefix(this._snapshot, id), callback);
    }
  });
  ...
  function(opts){
    ...
    if(typeof opts.index === 'string'){
      ...
      return H(db.createValueStream(
        prefix(this._index, idx, range(opts))
      ));
    }else{
      return H(db.createValueStream(
  ...
  function(){
    return H(db.createReadStream(
      ...
      var key = data.key.slice(data.key.indexOf('!', 1) + 1);
  ...
  function readChangesStream(current, limit){
    ...
    .map(function(at){
      at.key = at.key.slice(at.key.indexOf('!', 1) + 1);
      return at;
    })
    .reject(function(at){
      return (
  ...
  function(opts){
    ...
    function stream(clocks){
      var current = clocks.length > 0 ? clocksObject(clocks) : null;
      ...
      return H(changesStream)
        .map(H.wrapCallback(function(data, cb){
          ...
          self.mid(function(err, local){
            ...
            db.put(prefix(self._queue, seqKey(data._rev)), data, cb);
          });
        }))
        .parallel(1)
  ...
  Roda.fn.define('_replicate', params('result'), function(ctx, next, end){
    ...
    tx.get(prefix(self._clock, mid), function(err, curr){
      ...
      tx.del(prefix(self._queue, seqKey(ctx.result._rev)));
  ...
  Roda.fn.define('_conflicted', params('result'), function(ctx, done){
    ...
    tx.del(prefix(this._queue, seqKey(doc._rev)));
    tx.put(
