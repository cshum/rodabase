...
module.exports = function(path, opts){
  ...
  Roda.fn.define('get', params('id','deleted:boolean?','tx?'), function(ctx, done){
    ...
    if(ctx.params.tx){
      ...
      tx.get(prefix(this._snapshot, codec.encode(id)), callback);
    }else{
      db.get(prefix(this._snapshot, codec.encode(id)), callback);
    }
  });
  ...
  function operation(ctx, next){
    ...
    this.get(ctx.result._id, true, tx, function(err, val){
      ...
      tx.get(prefix(self._clock, mid), function(err, curr){
        ...
        //clean up timeline
        if(ctx.meta.time)
          tx.del(prefix(self._timeline, codec.encode(ctx.meta.time)));

        ctx.meta.time = timestamp();
        var time = extend({}, ctx.result);
        time._time = ctx.meta.time;

        //put timeline
        tx.put(prefix(self._timeline, codec.encode(ctx.meta.time)), time);
  ...
  Roda.fn.use('diff', function(ctx, next){
    ...
    function emit(name, key, value, unique){
      ...
      var idx = [key];
      if(unique === true){
        tx.get(prefix(self._index, name, codec.encode(key)), function(err, val){
