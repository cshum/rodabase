...
module.exports = function(path, opts){
  ...
  Roda.fn.define('mid', function(ctx, done){
    if(this._mid) return done(null, this._mid);
    //init transaction
    var tx = Roda.transaction();
    var mid;

    var self = this;
    tx.get(section(self._meta, 'mid'), function(err, val){
      if(val){
        mid = val;
        self._mid = mid;
        return;
      ...
      tx.defer(function(cb){
        randomBytes(MID_LEN, function(err, buf){
          ...
          tx.put(section(self._meta, 'mid'), mid);
          cb();
        });
      });
    });
