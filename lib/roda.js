...
module.exports = function(path, opts){
  ...
  Roda.fn.define('merger', function(ctx, next, end){
    ...
    H(this._merges.createValueStream({limit: 1}))
      .collect()
      .pull(function(err, arr){
        if(err) 
          return next(err); //stops on error
        else if(arr.length === 0)
          return next(true); //done if no more item on wait list
        else{
          ctx.result = arr[0];
          next();
        }
    ...
    if(curr){
      ...
      var mid = ctx.result._rev.slice(0,8);
      clocks.get(this._name + mid, function(err, now){
        if(!now || now < curr){
          return next(true); //not ready yet
        }else if(curr < now){
          //already in store, delete then keep going
          return self._merges.del(util.encodeClock(ctx.result._rev), next);
        }else{
          next();
        }
      });
    ...
    var self = this;
    this.merge(ctx.result, function(err){
      if(err){
        //result conflict
        var tx = Roda.transaction();
        tx.defer(function(cb){
          self.conflict(ctx.result, tx, cb);
        });
