        ...
        H(db.createValueStream(
          ...
          if(arr.length > 0){
            ...
            next(H('_write', self).map(function(state){
              return extend({ _time: state.time }, state.snapshot);
            }));
          }
        });
      }) : H(db.createValueStream(
  ...
  function operation(ctx, next){
    ...
    this.get(ctx.result._id, true, tx, function(err, state){
      ...
      tx.get(prefix(self._clock, mid), function(err, curr){
        ...
        //clean up timeline
        if(state.time)
          tx.del(prefix(self._timeline, codec.encode(state.time)));

        state.time = timestamp();

        //put timeline
        tx.put(
          prefix(self._timeline, codec.encode(state.time)), 
          extend({ _time: state.time }, ctx.result)
        );
