    ...
    this.get(ctx.result._id, tx, function(err, val){
        ...
        if(ctx.local){
          ...
          if(ctx.result._prev !== ctx.current._rev && 
            ...
            if(res > curr){
              ...
              //result conflict: change nothing, rollback.
              return next(new Error('Result conflict.'));
            }
          }
        }
      ...
      tx.defer(function(cb){
        ...
        self.diff(
          !!ctx.local,
          ctx.current || null, 
          H.extend(ctx.result, {}), //clone result
          tx, cb
        );
    ...
    H(this._merges.createValueStream())
      .map(H.wrapCallback(function(doc, cb){
        self.merge(doc, function(err){  
          if(err){
            if(err.notReady) cb(true); //error: time not ready, stop queue
            else self._merges.del(util.timeIndex(doc), cb); //error: delete merge
          }else cb(null);
        });
      }))
      .parallel(1)
