...
module.exports = function(path, opts){
  ...
  function Roda(name){
    ...
    this._snapshot = store.sublevel('snap');
    ...
    this._rootClocks = store.sublevel('roots');
    this._clocks = store.sublevel('clocks');

    this._changes = store.sublevel('changes');
    this._time = store.sublevel('time');

    this._mapper = null;

    var self = this;

    this._time.post(function(data){
      if(data.value)
  ...
  Roda.fn.define('get', params('id','snapshot:boolean?','tx?'), function(ctx, done){
    var id = String(ctx.params.id).trim();
    var store = ctx.params.snapshot ? this._snapshot : this.store;
    ...
    if(ctx.params.tx){
      ...
      tx.get(util.encode(id), {
        prefix: store
      }, function(err, doc){
        if(err && err.notFound)
          return done(error.notFound(id), null);
        if(err) 
          return done(err);
      ...
      store.get(util.encode(id), function(err, doc){
        if(err && err.notFound)
          return done(error.notFound(id), null);
        if(err) 
          return done(err);
  ...
  function(opts){
    ...
    return (
      live ? H(function(push, next){
        H(self._time.createReadStream(range(opts)))
          .stopOnError(push)
      ...
      }) : H(this._time.createReadStream(range(opts)))
    ).map(function(data){
  ...
  function operation(ctx, next){
    ...
    this.get(ctx.result._id, true, tx, function(err, val){
      ...
      if(ctx.current){
        ...
        if(ctx.result._rev){
          ...
          if(ctx.result._from !== ctx.current._rev && 
            ...
            (!ctx.current._merge || ctx.result._rev.indexOf(
              ctx.current._merge.slice(0, MID_LEN)) !== 0
            )){
      ...
      tx.get(mid, {
        ...
        //clock update
        tx.put(mid, stamp, { prefix: self._rootClocks });
        tx.put(mid, stamp, { prefix: self._clocks });
        ...
        tx.put(enId, ctx.result, { prefix: self._snapshot });
  ...
  Roda.fn._resolver = function(doc){
    return util.stampKey(doc._merge || doc._rev);
  };
  ...
  function readChangesStream(current, limit){
    ...
    var count = 0;
    return H(
      this._rootClocks.createReadStream()
    ).reject(function(at){
      return (
      ...
      return H(changesStream)
        .map(H.wrapCallback(function(data, cb){
          ...
          self.mid(function(err, local){
            ...
            self._waitlist.put(util.stampKey(data._rev), data, cb);
          });
        }))
        .parallel(1)
  ...
  Roda.fn.define('_replicate', params('result'), function(ctx, next, end){
    ...
    if(ctx.result._merge === ctx.result._rev){
      ...
      //join stamp follow casual ordering
      tx._stamp = Math.max(
        tx._stamp || 0, 
        util.decodeNumber(stamp)
      );
      tx.defer(function(cb){
        self.mid(function(err, mid){
          if(err) return next(err);
