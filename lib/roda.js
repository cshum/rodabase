...
module.exports = function(path, opts){
    ...
    this.get(ctx.result._id, tx, function(err, val){
      ...
      tx.get(mid, {
        ...
        if(ctx.meta.timestamp)
          tx.del(util.encode(ctx.meta.timestamp), {
            prefix: self._time
          });

        ctx.meta.timestamp = util.timestamp();
        tx.put(util.encode(ctx.meta.timestamp), ctx.result, {
          prefix: self._time
        });
  ...
  function(){
    return H('write', this).map(function(data){
