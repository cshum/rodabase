...
module.exports = function(path, opts){
  ...
  function Roda(name){
    ...
    this._name    = name;
    this._clocks  = this.store.sublevel('clocks');
    ...
    merger.call(this); //start merger whenever
  }
    ...
    return H.pipeline(function(changeStream){
      return H(changeStream)
        .map(H.wrapCallback(function(data, cb){
          ...
          tx.get(data._rev.slice(0,8), {
            prefix: self._clocks
          }, function(err, time){
            if(!time || data._rev.slice(8) > time)
              tx.put(util.encodeClock(data._rev), data, {
                prefix: self._merges
              });
          });
          tx.commit(cb);
  ...
  function merger(){
    ...
    H(this._merges.createValueStream())
      .map(H.wrapCallback(function(doc, cb){
        var curr = doc._curr;
        var mid = doc._rev.slice(0,8);
        self._clocks.get(mid, function(err, time){
          if(!time && !curr || time === curr){
            return cb(null, doc);
          }else if(!time || time < curr){
            //time not ready yet
            return cb(true);
          }else if(time > curr){
            //already in store. Delete then next
            return self._merges.del(util.encodeClock(doc._rev), cb);
          }
        });
      }))
      .parallel(1)
      ...
      .map(H.wrapCallback(function(doc, done){
        self.merge(doc, function(err){
          if(err) self.mergeError(doc, done);
          else done(null);
        });
      }))
      .parallel(1)
  ...
  Roda.fn.define('merge', params(
    ...
    ctx.transaction.del(util.encodeClock(ctx.result._rev), {
      prefix: this._merges
    });
    ...
    next();
  }, diff, function(ctx, done){
    ctx.transaction.commit(done);
  });

  Roda.fn.define('mergeError', params('result'), function(ctx, done){
    var tx = Roda.transaction();
    var doc = ctx.params.result;
    var self = this;
    tx.defer(function(cb){
      self.conflict(doc, tx, cb);
    });
    tx.del(util.encodeClock(doc._rev), {
      prefix: self._merges
    });
    tx.commit(done);
  });
