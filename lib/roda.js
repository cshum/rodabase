...
module.exports = function(path, opts){
  ...
  function Roda(name){
    ...
    this._repls = store.sublevel('repls');
      ...
      if(ctx.current){
        ...
        if(ctx.result._rev){
          ...
          if(ctx.result._from !== ctx.current._rev && 
            ...
            ctx.result._rev.indexOf(ctx.current._rev.slice(0,8)) !== 0 ){
  ...
  function readChangesStream(current, limit){
    ...
    return H(this.clockStream())
      ...
      .reject(function(at){
        return at.mid in current && at.time <= current[at.mid];
      })
      .map(function(at){
        return H(self._changes.createValueStream({
          gt: at.mid + (current[at.mid] || ''),
          lt: at.mid + '~'
        }));
      })
      .parallel(1)
  ...
  function(opts){
    ...
    return H.pipeline(function(changesStream){
      return H(changesStream)
        ...
        .parallel(1)
        .each(replicateWorker.bind(self, 1));
    });
  };

  function replicateWorker(replicated){
    if(this._working){
      this._replicated += replicated || 0;
      return;
    } 
    this._working = true;
    this._replicated = 0;
