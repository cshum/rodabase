...
module.exports = function(path, opts){
  ...
  Roda.fn.define('merger', function(ctx, next, end){
    var self = this;
      ...
      self._merging = false;
      if(done === true && ctx.result){
        self._merges.del(util.encodeClock(ctx.result._rev), function(err){
          if(!err) self.merger(); //keep running until done
        });
      }
    });
    ...
    if(this._clocksObj){
      ...
      this.clockStream().collect().pull(function(err, clocks){
        if(err) return next(err);
        ctx.clocksObj = util.clocksObject(clocks);
        next();
      });
  }, function(ctx, next){
