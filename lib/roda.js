var ginga        = require('ginga'),
    ...
    transactions = require('level-transactions'),
...
module.exports = function(path, opts){
  ...
  function Roda(name){
    ...
    this._snapshot = store.sublevel('ss');
    this._meta = store.sublevel('mt');
    this._timeline = store.sublevel('tl');
    this._changes = store.sublevel('ch');
    this._index = store.sublevel('i');

    this._clock = store.sublevel('ck');
    this._trigger = store.sublevel('tr');
    this._queue = store.sublevel('q');
  ...
  function operation(ctx, next){
    ...
    this.get(ctx.result._id, true, tx, function(err, val){
        ...
        if(ctx.result._rev){
          ...
          if(ctx.result._from !== ctx.current._rev && 
            ...
            if(self.resolver(ctx.result) > self.resolver(ctx.current)){
              ...
              tx.del(ctx.current._rev, { prefix: self._changes });
            }else{
      ...
      tx.get(mid, {
        ...
        //clock update
        tx.put(mid, stamp, { prefix: self._clock });
        ...
        //put changes
        tx.put(ctx.result._rev, ctx.result, { prefix: self._changes });

        //clean up timeline
        if(ctx.meta.time)
          tx.del(ctx.meta.time, { prefix: self._timeline });

        ctx.meta.time = util.encode(util.timestamp());

        //put timeline
        tx.put(ctx.meta.time, ctx.result, { prefix: self._timeline });
        ...
        tx.defer(function(cb){
          self.diff(ctx.meta, ctx.current, ctx.result, tx, function(err, meta){
            ...
            //put meta
            tx.put(ctx.result._id, meta, { prefix: self._meta });
            cb();
          });
        });
  ...
  function(){
    return H(this._clock.createReadStream()).map(function(data){
  ...
  function readChangesStream(current, limit){
    ...
    var self = this;
    var count = 0;
    return H(this._clock.createReadStream())
    .reject(function(at){
      return (
  ...
  Roda.fn.trigger = function(name, job, opts){
    ...
    this._triggered[name] = H(function(push, next){
      self._trigger.get(name, function(err, time){
    ...
    .parallel(parallel)
    .map(H.wrapCallback(
      this._trigger.put.bind(this._trigger, name)
    ))
    .parallel(1);
