...
module.exports = function(path, opts){
    ...
    ctx.transaction.defer(function(cb){
      self.mid(function(err, mid){
        ...
        if('id' in ctx.params)
          ...
          ctx.result._id = util.encode64(util.timestamp()) + ctx.local;
  ...
  function diff(ctx, next){
    ...
    tx.get(util.encode(ctx.result._id), {
      ...
      if(ctx.current){
        ...
        tx.del(util.encodeRev(ctx.current._rev), {
        ...
        if(ctx.local){
          ...
          if(ctx.result._from !== ctx.current._rev && 
            ...
            var res  = self._resolveBy(ctx.result);
            var curr = self._resolveBy(ctx.current);
      ...
      if(ctx.local){
        tx.get(self._name + ctx.local, {
          ...
          if(val)
            ctx.clock = ctx.local + val;

          var time64 = util.time64(Math.max(
            val ? util.time(val) : 0,
          ...
          //generate rev id
          ctx.result._rev = ctx.local + time64;
        });
      }
      ...
      tx.defer(function(cb){
        ...
        var time64 = ctx.result._rev.slice(8);

        //clock update
        tx.put(self._name + mid, time64, {
          prefix: clocks
        });
  ...
  Roda.fn._resolveBy = function(doc){
    return util.encodeRev(doc._rev);
  };
