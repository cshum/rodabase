...
module.exports = function(path, opts){
  ...
  function Roda(name){
    ...
    this._remotes = store.sublevel('remotes');
  ...
  function operation(ctx, next){
    ...
    this.get(ctx.result._id, tx, function(err, val){
      ...
      if(ctx.current){
        ...
        if(ctx.mid){
          //local write
          if(ctx.current._rev.slice(0,8) !== ctx.mid)
      ...
      //lock clock
      var mid = ctx.mid || ctx.result._rev.slice(0,8);
      var time;
      tx.get(mid, {
        prefix: self._clocks
      }, function(err, curr){
        if(ctx.mid){
          ...
          //genreate new _rev based on current clock and _rev
        }else{
          time = ctx.result._rev.slice(8);
        }
  ...
  function readChangesStream(checkpoint, limit){
    if(!checkpoint)
    ...
    function stream(clocks){
      ...
      return (
        live ? H(function(push, next){
          ...
          readChangesStream.call(self, checkpoint, 1)
            .stopOnError(push)
        ...
        }) : readChangesStream.call(self, checkpoint, limit)
      )
      .map(function(data){
        if(!('_rev' in data))
          return data;
  ...
  Roda.fn.define('replicate', params('result'), function(ctx, next, end){
    ...
    var remoteMid = ctx.result._rev.slice(0,8);
    var remoteTime = ctx.result._rev.slice(8);

    end(function(err){
      tx.release(err); 
    });

    if(ctx.result._merge === true){
      //merge overrrides mid and gets from
      tx._time = Math.max(
        tx._time || 0, 
        util.decodeNumber(remoteTime)
      );
      tx.defer(function(cb){
        self.mid(function(err, mid){
          if(err) return next(err);
    ...
    //gets from check
    if(ctx.result._from){
      var fromMid = ctx.result._from.slice(0,8);
      var fromTime = ctx.result._from.slice(8);
      tx.defer(function(cb){
        tx.get(fromMid, {
          prefix: self._clocks
        }, function(err, fromCurr){
          //to replicate clock must up to _from time
          if(!fromCurr || fromCurr < fromTime)
            return cb(error.NOT_READY);
          cb();
        });
      });
    }
    ...
    tx.get(remoteMid, {
      ...
      if(curr > last || curr >= remoteTime)
        return tx.commit(next);

      delete ctx.result._last;
      delete ctx.result._merge;

      tx.put(remoteMid, remoteTime, {
        prefix: self._remotes
      });
