...
R.define('get', params('id:string','tx:object?'), function(ctx, done){
  if(ctx.params.tx && ctx.params.tx.db === this.base.db){
    ctx.params.tx.get(
      util.encode(ctx.params.id), { prefix: this.store }, done
    );
  }else{
    this.store.get(util.encode(ctx.params.id), done);
  }
});
...
function current(ctx, next){
  this.get(ctx.result._id, ctx.transaction, function(err, res){
    if(res){
      ctx.current = res;
      ctx.transaction.del(
        util.encode(ctx.node, ctx.current._rev),
...
function put(ctx, next){
  ctx.transaction
    .put(
      util.encode(ctx.result._id), 
      ctx.result, 
      { prefix: this._store }
    )
    .put(
      util.encode(ctx.node, ctx.result._rev), 
      ctx.result,
      { prefix: this._changes }
    );
  next();
