...
module.exports = function(path, opts){
  ...
  function Roda(name){
    ...
    this._masters = store.sublevel('masters');
  ...
  function operation(ctx, next){
    ...
    this.get(ctx.result._id, tx, function(err, val){
      ...
      tx.get(mid, {
        ...
        //clock update
        tx.put(mid, time, { prefix: self._masters });
        tx.put(mid, time, { prefix: self._repls });
  ...
  function(opts){
    opts = opts || {};
    var stream = opts.master ? this._masters : this._repls;
    return H(stream.createReadStream()).map(function(data){
  ...
  function readChangesStream(current, limit){
    ...
    return H(this.clockStream({
      ...
      return {
        mid: curr.slice(0, MID_LEN),
        time: curr.slice(MID_LEN)
      };
    }).reject(function(at){
