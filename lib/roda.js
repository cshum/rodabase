...
module.exports = function(path, opts){
  ...
  Roda.fn.define('read', pull(Roda.fn.readStream));
  ...
  Roda.fn.define('clock', pull(Roda.fn.clockStream));
  ...
  function(since, limit){
    ...
    function stream(since){
      if(!since || since.length === 0)
        return H(
          self._changes.createValueStream({
            limit: limit - count
          })
        );

      since = util.clockObj(since);
    ...
    return H.pipeline(function (s) {
      return s.collect().map(stream).parallel(1);
    });
  };

  Roda.fn.define('changes', pull(Roda.fn.changesStream));
