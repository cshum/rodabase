...
module.exports = function(path, opts){
  ...
  Roda.fn.define('mid', function(ctx, next, end){
    ...
    var self = this;
    ctx.transaction.get(p('mid', this._name), function(err, val){
      if(val){
        ctx.mid = val;
        self._mid = ctx.mid;
        return next();
      ...
      ctx.transaction.defer(function(cb){
        randomBytes(MID_LEN, function(err, buf){
          ...
          ctx.transaction.put(p('mid', self._name), ctx.mid);
          next();
  ...
  Roda.fn.define('get', params('id','deleted:boolean?','tx?'), function(ctx, done){
    ...
    if(ctx.params.tx){
      ...
      tx.get(p(this._snapshot, util.encode(id)), callback);
    }else{
      db.get(p(this._snapshot, util.encode(id)), callback);
    }
  });
  ...
  function(opts){
    ...
    return H(db.createValueStream(
      typeof opts.index === 'string' ? 
        p(this._index, opts.index, range(opts)) :
        p(this._read, range(opts))
    ));
  };
  ...
  function operation(ctx, next){
    ...
    tx.get(p(self._meta, ctx.result._id), function(err, meta){
      ctx.meta = meta || {};
    });
    ...
    this.get(ctx.result._id, true, tx, function(err, val){
        ...
        if(ctx.result._rev){
          ...
          if(ctx.result._from !== ctx.current._rev && 
            ...
            if(self.resolver(ctx.result) > self.resolver(ctx.current)){
              ...
              tx.del(p(self._changes, ctx.current._rev));
            }else{
      ...
      var stamp;
      tx.get(p(self._clock, mid), function(err, curr){
        //return IO/other errors
        if(err && !err.notFound) 
          return next(err);
        ...
        //clock update
        tx.put(p(self._clock, mid), stamp);

        var enId = util.encode(ctx.result._id); 
        //put snapshot
        tx.put(p(self._snapshot, enId), ctx.result);
        if(ctx.result._deleted){
          //store del
          tx.del(p(self._read, enId));
        }else{
          //store put
          tx.put(p(self._read, enId), ctx.result);
        }
        //put changes
        tx.put(p(self._changes, ctx.result._rev), ctx.result);

        //clean up timeline
        if(ctx.meta.time)
          tx.del(p(self._timeline, ctx.meta.time));
        ...
        //put timeline
        tx.put(p(self._timeline, ctx.meta.time), time);
        ...
        tx.defer(function(cb){
          self.diff(ctx.meta, ctx.current, ctx.result, tx, function(err, meta){
            ...
            //put meta
            tx.put(p(self._meta, ctx.result._id), meta);
            cb();
          });
        });
  ...
  Roda.fn.use('diff', function(ctx, next){
    ...
    if(ctx.meta.indexed){
      for(name in ctx.meta.indexed){
        ...
        for(i = 0, l = keys.length; i < l; i++){
          tx.del(p(self._index, name, keys[i]));
        }
      }
    }
    ctx.meta.indexed = {};
    ...
    function emit(name, key, value, unique){
      ...
      if(unique === true){
        tx.get(p(self._index, name, enKey), function(err, val){
          if(val)
      ...
      tx.put(p(self._index, name, enKey), extend({}, value || result, {
        _id: ctx.result._id,
        _key: key
      }));
  ...
  function(){
    return H(db.createReadStream(p(this._clock, {}))).map(function(data){
      ...
      return H(changesStream)
        .map(H.wrapCallback(function(data, cb){
          ...
          self.mid(function(err, local){
            ...
            db.put(p(self._queue, util.stampKey(data._rev)), data, cb);
          });
        }))
        .parallel(1)
  ...
  function replicateWorker(replicated){
    ...
    var self = this;
    H(db.createValueStream(p(this._queue, {})))
  ...
  Roda.fn.define('_replicate', params('result'), function(ctx, next, end){
    ...
    if(ctx.result._from){
      ...
      tx.defer(function(cb){
        tx.get(p(self._clock, fromMid), function(err, fromCurr){
    ...
    tx.get(p(self._clock, mid), function(err, curr){
      ...
      tx.del(p(self._queue, util.stampKey(ctx.result._rev)));
  ...
  Roda.fn.define('_conflicted', params('result'), function(ctx, done){
    ...
    tx.del(p(this._queue, util.stampKey(doc._rev)));
    tx.put(
      p(this._clock, doc._rev.slice(0, MID_LEN)), 
      doc._rev.slice(MID_LEN)
    );
    tx.commit(done);
  });
  ...
  Roda.fn.trigger = function(name, job, opts){
    ...
    this._triggered[name] = H(function(push, next){
      db.get(p(self._trigger, name), function(err, time){
    ...
    .parallel(parallel)
    .map(H.wrapCallback(
      db.put.bind(db, p(this._trigger, name))
    ))
    .parallel(1);
