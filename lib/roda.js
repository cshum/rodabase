...
module.exports = function(path, opts){
  ...
  Roda.fn.define('merge', params('result'), function(ctx, next, end){
    ...
    var mergeMid = ctx.result._rev.slice(0,8);
    var mergeTime = ctx.result._rev.slice(8);

    //lamport timestamp gets-from merge
    tx._time = Math.max(
      tx._time || 0, util.decodeNumber(mergeTime)
    );
