...
module.exports = function(path, opts){
    ...
    this.get(ctx.result._id, tx, function(err, val){
      ...
      if(ctx.current){
        if(ctx.noCurrent === true)
          return next(error.keyExists(ctx.result._id));
        ...
        if(ctx.withCurrent === true)
          return next(error.keyNotFound(ctx.result._id));
  ...
  Roda.fn.define('put', params(
    'id:string?','result:object','tx?'), local, operation, write);

  Roda.fn.define('del', params(
    'id:string','tx?'), local, operation, write);
