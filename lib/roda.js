...
module.exports = function(path, opts){
  ...
  function operation(ctx, next){
    ...
    this.get(ctx.result._id, tx, function(err, val){
      ...
      tx.get(mid, {
        ...
        //clock update
        tx.put(mid, time, { prefix: self._rootClocks });
  ...
  Roda.fn._resolver = function(doc){
    return util.timeIndex(doc._merge || doc._rev);
  };
  ...
  Roda.fn.define('replicate', params('result'), function(ctx, next, end){
    ...
    tx.get(mid, {
      ...
      tx.del(util.timeIndex(ctx.result._rev), {
        prefix: self._queue
      });
  ...
  Roda.fn.define('conflicted', params('result'), function(ctx, done){
    ...
    tx.del(util.timeIndex(doc._rev), {
      prefix: this._queue
    });
    tx.put(
