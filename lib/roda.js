...
module.exports = function(path, opts){
  ...
  Roda.fn.define('put', params('id:string?','result:object','tx?'),
    prepare, validate, mid, current, rev, diff, invoke);

  Roda.fn.define('del', params('id:string','tx?'),
    prepare, mid, current, rev, diff, invoke);
  ...
  function current(ctx, next){
    ...
    ctx.transaction.get(util.encode(ctx.result._id), {
      ...
      ctx.transaction.del(util.encodeRev(rev), {
  ...
  function readChangeStream(afterObj, limit){
    ...
    var from = null, mid;
    for(mid in afterObj){
      var en = util.encodeRev(mid + afterObj[mid]);
    ...
    return H(this._changes.createValueStream({
      ...
      return mid in afterObj && data._rev.slice(8) <= afterObj[mid];
    }).take(limit || null);
  }
  ...
  function(opts){
    ...
    var after = Array.isArray(opts.after) ? opts.after : null;

    var self = this;

    function stream(after){
      var afterObj = after && after.length > 0 ? util.clockObject(after) : null;
    ...
    return after ? stream(after) : H.pipeline(function (clockStream) {
