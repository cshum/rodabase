...
module.exports = function(path, opts){
  ...
  //Unique id for each Roda
  Roda.fn.define('id', params('tx?'), function(ctx, next, end){
    if(this._id)
      return next(null, this._id);
    //init transaction
    if(ctx.params.tx){
    ...
    ctx.transaction.get(this._name, {
      ...
      ctx.transaction.defer(function(cb){
        crypto.randomBytes(6, function(err, buf){
          ...
          ctx.id = buf.toString('base64')
            .replace(/\//g,'_')
            .replace(/\+/g,'-');
          if(ctx.id.length !== 8)
            return next(new Error('MID must be length 8'));

          ctx.transaction.put(self._name, ctx.id, {
            prefix: ids
          });
          next();
  ...
  Roda.fn.define('put', params('id:string?','result:object','tx?'),
    prepare, validate, id, current, rev, diff, put, invoke);
  ...
  function id(ctx, next){
    this.id(ctx.transaction, function(err, id){
      if(err)
        return next(err);
      ctx.id = id;
      next();
  ...
  function current(ctx, next){
    ...
    if('id' in ctx.params)
      ...
      ctx.result._id = util.encode64(
        util.timestamp() * 1000 + Math.random()
      ) + ctx.id;
    }
  ...
  function rev(ctx, next){
    ...
    var key = this._name + ctx.id;
    ctx.transaction.get(key, {
      prefix: clocks
    }, function(err, val){
      if(val)
        ctx.clock = ctx.id + val;
  ...
  function diff(ctx, next){
    ctx.change = H.extend(ctx.result, {});
    ctx.change._from = {};
    if(ctx.current)
      ctx.change._from.rev = ctx.current._rev;
    if(ctx.clock)
      ctx.change._from.clock = ctx.clock;
    var self = this;
  ...
  function put(ctx, next){
    ...
    ctx.transaction
      ...
      .put(ctx.result._rev, ctx.change, {
  ...
  function readChangeStream(sinceObj, limit){
    var count = 0;
    var self = this;
    if(!sinceObj)
      return H(self._changes.createValueStream({ limit: limit }));

    return self.clockStream()
      .map(function(data){
        var mid = data.slice(0, 8);
        return {
          mid:  mid,
          from: sinceObj[mid],
          to:   data.slice(8)
      ...
      .filter(function(info){
        return !info.from || info.from < info.to;
      })
      .reject(function(){
      ...
      .map(function(info){
        return H(self._changes.createValueStream({
          gt: info.mid + info.from,
          lt: info.mid + '~',
          limit: limit > -1 ? limit - count : -1
        }));
      })
      .parallel(1)
      .map(function(data){
  ...
  function(opts){
    ...
    function stream(since){
      var sinceObj = since && since.length > 0 ? util.clockObject(since) : null;

      return live ? H(function(push, next){
        //live changes = readChangeStream + liveStream
        readChangeStream.call(self, sinceObj, 1)
          .collect()
          .pull(function(err, arr){
            ...
            if(err){
              ...
              var doc = arr[0];
              sinceObj = sinceObj || {};
              sinceObj[doc._rev.slice(0,8)] = doc._rev.slice(8);
              push(null, doc);
              next();
            }else{
      ...
      }): readChangeStream.call(self, sinceObj, limit);
    }
    return since ? 
      stream(since) : 
