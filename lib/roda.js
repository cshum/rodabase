...
function node(ctx, next){
  ctx.transaction.get('node', function(err, value){
    //must get node id before putting any revision
    if(err) return next(err);
...
function current(ctx, next){
  this.get(ctx.result._id, ctx.transaction, function(err, res){
    if(res){
      ctx.current = res;
      ctx.transaction.del(
        encode(ctx.node, ctx.current._rev),
...
function put(ctx, next){
  ctx.transaction
    ...
    .put(
      encode(ctx.node, ctx.result._rev), 
      ctx.result,
      { prefix: this._changes }
    );
  next();
...
R.define(
  ...
  validate, tx, node, current, diff, put, invoke
);
R.define(
  'del', params('id:string','tx:object?'),
  tx, node, current, diff, del, invoke
);
