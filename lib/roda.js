...
module.exports = function(path, opts){
  ...
  function(index, opts){
    opts = opts || {};
    if(typeof index === 'string') opts.index = index;
    else opts = index || {};
  ...
  Roda.fn.define('read', util.pull(Roda.fn.readStream));
  ...
  function diff(ctx, next){
    ctx.changes = {};
    if(ctx.current)
      ctx.changes.rev = ctx.current._rev;
    if(ctx.clock)
      ctx.changes.clock = ctx.clock;
    ctx.changes.result = ctx.result;
  ...
  function put(ctx, next){
    ...
    ctx.transaction
      ...
      .put(ctx.result._rev, ctx.changes, {
  ...
  function(){
    return H(clock.createReadStream({
  ...
  Roda.fn.define('clock', util.pull(Roda.fn.clockStream));

  Roda.fn.changesStream = 
  Roda.fn.createChangesStream = 
  function(since, limit){
    ...
    return since ? 
      stream(since) : 
      H.pipeline(function (sinceStream) {
        return sinceStream
          .collect()
          .map(stream)
          .parallel(1);
  ...
  Roda.fn.define('changes', params(
    'clock:array?', 'limit:number?'
  ), function(ctx, done){
    this.changesStream(ctx.params.clock || [], ctx.params.limit)
      .collect()
      .pull(done);
  });
