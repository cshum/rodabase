...
module.exports = function(path, opts){
  ...
  // Roda.fn.define('changes', pull(Roda.fn.clockStream));

  Roda.fn.define('changes', params(
    'since:array?','limit:number?'
    ...
    function receive(mid, start, cb){
      ...
      H(self._changes.createValueStream(opts))
        .map(function(data){
          count++;
          result.push(data);
          return data;
    ...
    if(!ctx.params.since){
      ...
      this.clock(function(err, data){
        var clock = util.clockObj(data);
        var since = util.clockObj(ctx.params.since);
        var q = util.queue();
