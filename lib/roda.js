  ...
  function merger(){
    ...
    H(this._merges.createValueStream())
      .map(H.wrapCallback(function(doc, cb){
        ...
        clocks.get(self._name + mid, function(err, now){
          if(!now && !curr || now === curr){
            return cb(null, doc);
          }else if(!now || now < curr){
            return cb(true); //not ready yet
          }else if(now > curr){
            return self._merges.del(util.encodeClock(doc._rev), cb); //delete then next
          }
        });
      }))
      .parallel(1)
