...
module.exports = function(path, opts){
  ...
  function(opts){
    ...
    var clocks = Array.isArray(opts.clocks) ? opts.clocks : [];
  ...
  function(opts){
    ...
    var plex = multiplex(function (stream, id) {
      stream = stream.pipe(es.split()).pipe(es.parse());
      switch(id){
        case 'clock': //read changes
        H(stream).collect().through(function(clocks){
          console.log(clocks);
          self.changesStream({
            clocks: clocks, live: true
          })
          .pipe(es.stringify())
          .pipe(plex.createStream('changes'));
        });
        break;
        case 'changes': //queue changes
        H(stream).map(H.wrapCallback(function(data, cb){
