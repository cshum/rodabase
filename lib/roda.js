...
module.exports = function(path, opts){
  ...
  Roda.fn.insert = Roda.fn.post;
  Roda.fn.upsert = Roda.fn.put;
  ...
  function readChangesStream(clocksObj, mid, limit){
    if(mid)
      return H(this._changes.createValueStream({
        gt: mid + (clocksObj && clocksObj[mid] ? clocksObj[mid] : ''),
        lt: mid + '~',
        limit: limit || -1
      }));
  ...
  function(opts){
    ...
    function changesStream(clocks){
      var clocksObj = clocks.length > 0 ? util.clocksObject(clocks) : null;
      ...
      .map(function(mid){
        return live ? H(function(push, next){
          ...
          readChangesStream.call(self, clocksObj, mid, 1)
            .collect()
        ...
        }) : readChangesStream.call(self, clocksObj, mid, limit);
      })
      .parallel(1)
      .map(function(data){
        ...
        if(mid in clocksObj)
          data._last = clocksObj[mid];

        clocksObj[mid] = data._rev.slice(8);
        return data;
      });
    }
    return clocks ? changesStream(clocks) : H.pipeline(function (clockStream) {
      return clockStream.collect().map(changesStream).parallel(1);
    });
  };
