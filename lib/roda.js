...
module.exports = function(path, opts){
  ...
  Roda.fn.define('mid', function(ctx, next, end){
    ...
    var self = this;
    ctx.transaction.get(prefix(self._meta, 'mid'), function(err, val){
      if(val){
        ctx.mid = val;
        self._mid = ctx.mid;
        return next();
      ...
      ctx.transaction.defer(function(cb){
        randomBytes(MID_LEN, function(err, buf){
          ...
          ctx.transaction.put(prefix(self._meta, 'mid'), ctx.mid);
          next();
  ...
  Roda.fn.define('get', params('id','state:boolean?','tx?'), function(ctx, done){
    ...
    (tx || db).get(prefix(this._state, id), function(err, state){
      if(err && err.notFound)
  ...
  Roda.fn.define('find', params('key', 'index:string?'), function(ctx, done){
    if(ctx.params.index){
      this.readStream({ 
        ...
        if(err) return done(err);
        if(!(arr && arr.length)) 
          return done(error.notFound(ctx.params.key));
        done(null, arr[0]);
      });
    }else{
      this.get(ctx.params.key, done);
    }
  });
    ...
    return (
      live ? H(function(push, next){
        H(db.createValueStream(
          prefix(self._history, range(opts))
        )).collect().pull(function(err, arr){
        ...
        prefix(this._history, range(opts))
      ))
    );
  };
  ...
  function operation(ctx, next){
    ...
    this.get(ctx.result._id, true, tx, function(err, state){
      ...
      tx.get(prefix(self._clock, mid), function(err, curr){
        ...
        //clean up timeline
        if(state.time)
          tx.del(prefix(self._history, codec.encode(state.time)));

        state.time = timestamp();

        //put timeline
        tx.put(
          prefix(self._history, codec.encode(state.time)), 
        ...
        tx.defer(function(cb){
          //put state
          tx.put(prefix(self._state, ctx.result._id), state);
  ...
  Roda.fn.define('rebuildIndex', params('tag?'), function(ctx, next, end){
    if('tag' in ctx.params){
      db.get(prefix(this._meta, 'rebuild'), {
        valueEncoding:'utf8' 
      }, function(err, tag){
    ...
    this.readStream().map(H.wrapCallback(function(data, cb){
      ...
      self.get(data._id, true, tx, function(err, state){
        ...
        indexer.call(self, state, tx);
        //put state
        tx.put(prefix(self._state, data._id), state);
      });
      tx.commit(function(){
        cb();
      });
    ...
    .done(function(){
      if('tag' in ctx.params){
        db.put(prefix(self._meta, 'rebuild'), ctx.params.tag, done);
      }else{
        done(null);
      }
    });
  ...
  function(){
    return H(db.createReadStream(
      ...
      return prefix(data.key) + data.value;
    });
  };
      ...
      return H(changesStream)
        .map(H.wrapCallback(function(data, cb){
          ...
          self.mid(function(err, local){
            ...
            db.put(prefix(self._queue, codec.seqKey(data._rev)), data, cb);
          });
        }))
        .parallel(1)
  ...
  Roda.fn.define('_replicate', params('result'), function(ctx, next, end){
    ...
    tx.get(prefix(self._clock, mid), function(err, curr){
      ...
      tx.del(prefix(self._queue, codec.seqKey(ctx.result._rev)));
  ...
  Roda.fn.define('_conflicted', params('result'), function(ctx, done){
    ...
    tx.del(prefix(this._queue, codec.seqKey(doc._rev)));
    tx.put(
