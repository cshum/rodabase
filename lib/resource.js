...
function mid(ctx, next){
  this.roda.db.mid(function(err, mid){
    ...
    if('id' in ctx.params)
      ...
      ctx.result._id = util.encode(util.timestamp()) + ctx.mid;
    }
    next();
  });
}
...
//Index
R.index = function(name, mapper, cb){
  ...
  if(!this._maps[name] && typeof name === 'string' && 
    ...
    cb(new Error('Invalid index mapper'));
  }
  return this;
};
...
R.use('diff', function(ctx, next){
  ...
  ctx.transaction.get(ctx.id, {
    ...
    _.each(self._maps, function(mapper, name){
