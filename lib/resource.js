...
function Resource(roda, name){
  ...
  //can retrieve global clock without scanning through resources
  this._clock = roda.db.sublevel('clock');
    ...
    var time = util.trim(util.encode64(
      Math.max(
        ...
        ctx.current ? util.decode64(
          util.pad(ctx.current._rev.slice(MID_LEN), 12)
        ) : 0
      ) + 1
    ));
...
R.define('clock', function(ctx, done){
  ...
  this._clock.createReadStream({
    ...
    .on('data', function(data){
      arr.push(data.key.slice(-MID_LEN) + data.value);
    })
    .on('close', function(){
      done(null, arr.join(','));
    })
