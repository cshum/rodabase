...
R.use('diff', function(ctx, next){
  ...
  ctx.transaction.get(ctx.id, {
    ...
    if(indexed){
      for(name in indexed){
        ...
        for(i = 0, l = keys.length; i < l; i++){
          key = keys[i];
          ctx.transaction.del(util.encode(key), {
            prefix: this._indexes.sublevel(name)
          });
        }
      }
    }
    ...
    function emit(name, key, value, unique){
      ...
      // key: util.encode(key) + (unqiue ? '' : util.encode(util.timestamp())),
      indexes[name].push({
        key: key,
        value: value, //todo: default _key, _id mapping in value
        unique: unique === true
      });

      //todo: index mapper stuffs
    }
