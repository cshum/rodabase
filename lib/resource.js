...
function Resource(roda, name){
  ...
  this._indexes = this.store.sublevel('indexes');
...
R.index = function(name, mapper){
  ...
  if(!this._maps[name] && typeof name === 'string' && 
...
R.use('diff', function(ctx, next){
  ...
  var self = this;
  ctx.transaction.get(ctx.id, {
    prefix: this._indexed
  }, function(err, indexed){
    ...
    if(indexed){
      for(name in indexed){
        ...
        for(i = 0, l = keys.length; i < l; i++){
          ctx.transaction.del(keys[i], {
            prefix: self._indexes.sublevel(name)
          });
        }
      }
    }
    ...
    function check(name, enKey, cb){
      ctx.transaction.get(enKey, {
        prefix: self._indexes.sublevel(name)
      }, function(err, val){
        if(val) cb(new Error(util.decode(enKey)+' must be unique.'));
        else cb();
      });
    }
    ...
    function emit(name, key, value, unique){
      ...
      var enKey = util.encode(key) + (
        unqiue ? '' : util.encode(util.timestamp()) );

      //record encoded key
      indexes[name].push(enKey);

      //put index store
      ctx.transaction.put(enKey, _.extend(value || {}, {
        _id: ctx.id,
        _key: key
      }), {
        prefix: self._indexes.sublevel(name)
      ...
      //check unique
      if(unique) q.add(check.bind(null, enKey));
    }
    ...
    //record indexed keys
    ctx.transaction.put(ctx.id, indexes, {
      prefix: self._indexed
    });
    async = true;
